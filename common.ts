...
export function random(min = 0, max = 1, curve = 1): number {
...
export function circle(ctx: CanvasRenderingContext2D, x: number, y: number, r: number, method: "fill" | "stroke" = "fill") {
  ctx.beginPath();
  ctx.arc(x, y, r, 0, TAU);
  if (method === "fill") {
    ctx.fill();
